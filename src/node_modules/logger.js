...
const $reconnectionMaxDelay = Symbol('reconnectionInitialDelay property');
...
export default class Logger extends Writable {
	constructor(opts) {
		...
		this.revalidateConnectionPromise();
	}
	...
	async _write(log, enc, cb) {
		...
		try {
			...
			DEBUG.log("Got promise. Writing new log. Connection destroyed: " + conn.destroyed);
			conn.write(log);
		} catch (err) {
			this.emit('error', err);
			DEBUG.log("Error: " + err);
	...
	async [$getConnection]() {
		...
		if (this[$connection]) {
			...
			if (!conn.destroyed) {
				...
				DEBUG.log("Found connection promise but it is disconnected.");
				return this.revalidateConnectionPromise();
			}
		} else {
			DEBUG.log("No connection promise found. Creating a new one.");
			return this.revalidateConnectionPromise();
		}
	}

	revalidateConnectionPromise() {
		var reconnect = reconnect_core(function () {
			let connection;
			if (this.secure) {
				DEBUG.log("Secure connection flag found. Creating a secure connection.");
				connection = tls.connect.apply(opts, () => {
		...
		this[$connection] = new Promise((resolve, reject) => {
			...
			this[$reconnection].on('disconnect', function (err) {
				DEBUG.log("Disconnected: " + err);
