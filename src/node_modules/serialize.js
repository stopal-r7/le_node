...
const errReplacer = (val, withStack) => {
  ...
  let err = { name: val.name || 'Error', message: val.message };

  // Though custom errors could have some own properties:

  err = Object.assign(err, val);
...
const flat = (serialize, arraysToo) =>
    (obj) => {
      const newObject = JSON.parse(serialize(obj));

      if (!_.isObject(newObject)) return newObject;

      const flatObj = _.reduce(newObject, function _flat(target, values, key) {
        ...
        if (!_.isObject(values)) {
          newTarget[joinedKey] = values;
        } else if (!arraysToo && _.isArray(values)) {
          newTarget[joinedKey] = values.map(val => {
            if (!_.isObject(val)) return val;

            return _.reduce(val, _flat, {}, []);
          });
        } else {
          _.reduce(values, _flat, newTarget, keyContext);
        }
        return newTarget;
      }, {}, []);
...
const build = (flatten, flattenArrays, replacer = pass, withStack) => {
