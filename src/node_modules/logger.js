...
export default class Logger extends Writable {
	constructor(opts) {
		...
		this[$secure]                   = opts.secure || defaults.secure;
		...
		this[$reconnect] = reconnect_core(function () {
			var connection;
			let secure = opts.secure == true ? true : opts.secure == false ? false : defaults.secure;
			if (secure) {
	...
	@enumerable
	closeConnection() {
		DEBUG.log("Closing connection and retry mechanism.");
		if (!this[$connection])
			return;
		...
		// this makes sure retry mechanism and connection will be stopped.
	...
	getConnectionPromise() {
		...
		this[$reconnection].once('disconnected', () => {
			// Destroying broken promise. We don't need it anymore. Bitter earth.
			delete this[$connection];
		});
