...
export default class Logger extends Writable {
  constructor(opts) {
    ...
    this.ringBuffer = new RingBuffer(opts);
    ...
    this[$reconnect] = reconnectCore(function () {
      let connection;
      if (secure) {
  ...
  log(lvl, log) {
    ...
    // if RingBuffer.write returns false, don't create any other write request
    // for the writable stream to avoid memory leak
    // this means there are already 'bufferSize' of write events in the
    // writable stream and that's what we want.
  ...
  [$toLevel](val) {
    ...
    if (levelUtil.isNumberValid(val)) {
      num = parseInt(parseInt(val, 10), 10);
    } else {
